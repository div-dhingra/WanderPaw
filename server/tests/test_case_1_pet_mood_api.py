...
def test_case_1_get_pet_mood():
    ...
    # Verify pet_mood object structure (i.e. all keys exists in response)
    assert "mood" in data
    assert "message" in data

    # Verify expected, specific values
    assert data["message"] == "Fetched pet mood!"

    pet_mood, last_updated = data.get("mood")
    ...
    # Verify that mood integer-attribute is within range (0 - 100)
