...
def test_case_1_update_pet_mood():
    ...
    response = client.patch("/api/users/andrews_covalent_bond/update-pet-mood", json=payload) # Send POST request
    ...
    response = client.get("/api/users/pet-mood?user_name=andrews_covalent_bond")
    ...
    new_mood = data_2.get("mood")[0]

    # Assert key-value pairs in response
    assert data_1["message"] == f"User {user_name} pet mood status updated to {new_mood}"